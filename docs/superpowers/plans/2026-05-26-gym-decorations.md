# Gym Decoration Enhancement Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the voxel gym visually rich and impressive by adding warm point lights, ceiling beams with pendant fixtures, mirrors, lockers, rubber mats, tall corner palms, a speed bag, weight tree, towel rack, water cooler, floor stripe, and foam roller basket — all within the existing warm-wood aesthetic.

**Architecture:** All purely visual decorations live in a new `js/gym/decorations.js` module (no interactables, no colliders). Lighting and structural elements (beams, pendants, point lights) go in `js/gym/scene.js`. `js/main.js` gets one new import and one new function call. No existing file structure is changed.

**Tech Stack:** Three.js (importmap CDN), vanilla ES modules. No Vitest tests — all new code is browser-only Three.js geometry.
...
|---|---|---|
| `js/gym/scene.js` | Modify | Add 5 warm point lights, 1 cool fill light, 3 ceiling beams, 6 pendant fixtures |
| `js/gym/decorations.js` | Create | All purely visual decoration builders |
| `js/main.js` | Modify | Import and call `buildDecorations(scene)` |

---

## Task 1: Enhance scene.js — point lights, ceiling beams, pendant fixtures

**Files:**
- Modify: `js/gym/scene.js`

- [ ] **Step 1.1 — Replace `_buildLighting` with version that adds warm point lights**

In `js/gym/scene.js`, replace the entire `_buildLighting` function with:
...
- [ ] **Step 1.2 — Add ceiling beams and pendant fixtures inside `_buildWalls`**

In `js/gym/scene.js`, inside `_buildWalls`, add the following block **just before** the `_addPlants(threeScene)` call at the bottom of the function:
...
git commit -m "feat: add point lights, ceiling beams, and pendant fixtures to gym"
```

---

## Task 2: Create js/gym/decorations.js

**Files:**
- Create: `js/gym/decorations.js`

This file exports one function: `buildDecorations(scene)`. It has no interactables and no colliders — purely visual scenery. It uses `box()` from `voxel.js` and raw `THREE.Mesh` constructors.

- [ ] **Step 2.1 — Create the file with the export scaffold and mirrors**

Create `js/gym/decorations.js` with this content:
...
function _buildWeightTree(scene) {
  ...
  // Plate geometry: thin on x (axle), wide on y-z (disc face)
  [
...
Expected: 11 tests pass (decorations.js is browser-only, excluded from node test env).

- [ ] **Step 2.3 — Commit**

```bash
git add js/gym/decorations.js
git commit -m "feat: add decorations — mirrors, lockers, mats, palms, speed bag, weight tree, towel rack, water cooler, floor stripe, foam rollers"
```

---

## Task 3: Wire decorations into main.js
...
In `js/main.js`, after the last existing `import` line, add:

```js
import { buildDecorations } from './gym/decorations.js';
```

- [ ] **Step 3.2 — Call buildDecorations inside initGym**

In `window.__initGym`, find the line `buildScene(scene);` and add the call directly after it:
...
```bash
git add js/main.js
git commit -m "feat: wire buildDecorations into gym init"
```
...
- `ROOM` and `HALF` are defined as local constants in `decorations.js` (not imported from `scene.js`) to avoid coupling between modules.
- All pendant glow cores use `MeshBasicMaterial` — this material ignores scene lighting and renders at full brightness, creating the appearance of a glowing light source without adding render cost.
- Locker positions: body at x=9.56 (inside wall at x=10), door face at x=9.29 (facing into room). This places lockers flush against the right wall.
- Tall palms at (-8, -8.5) and (8, -8.5) are distinct from existing corner plants at (-9.2, -9.2) and (9.2, -9.2) — no overlap.
- Speed bag at (2.5, cz=-4.5 to -3.9) does not overlap with punching bag at (0, -3) or pull-up rig at (5, -2).
- Weight tree at (4.5, -8.5) is adjacent to dumbbell rack at (6, -7) — stethoscope is at (2, -8.5), leaving a comfortable gap.
- Water cooler at (4.5, 8.5) is near the entrance — not overlapping with any interactable props.
- Foam roller basket at (-4, 6.5) is in the stretching mat zone corner — near soccer ball at (-3, 4) but not overlapping.
- The `Object.assign` pattern used in `_buildWaterCooler` and `_buildFoamRollers` is a compact way to set `.position` on a mesh before adding to scene. Functionally identical to `mesh.position.set(...)`.
